...
def load_data_from_dir(instance_dir, image_size=256, pad_size=0.1, skip_indices=()):
    ...
    camera_dict = np.load(osp.join(instance_dir, "cameras_sphere.npz"))

    for i, image_path in enumerate(sorted(glob(osp.join(image_dir, "*.png")))):
        if i in skip_indices:
            continue
        image_name = osp.basename(image_path)
        mask_path = osp.join(mask_dir, image_name)
        ...
        num = str(int(image_name.split('.png')[0]))
        world_mat = camera_dict['world_mat_' + num].astype(np.float32)
        scale_mat = camera_dict['scale_mat_' + num].astype(np.float32)
